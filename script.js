...
/* ===== PWA: install prompt + service worker registration =====
   Append this block to the END of your existing script.js.
   Since script.js already loads on nearly every page, this alone makes
   the install button + offline caching work site-wide. */
...
window.addEventListener('beforeinstallprompt', (event) => {
  // Chrome/Android fires this when the site meets install criteria.
  // We stop it from showing its own mini-bar and drive our own button
  // instead, so we control when/how it's offered.
...
function showInstallButton() {
  if (document.getElementById('shs-install-btn')) return; // already shown

  const btn = document.createElement('button');
  btn.id = 'shs-install-btn';
  ...
  // Don't nag forever - hide it after 20s if ignored, it'll still be
  // installable via the browser's own menu (Chrome: ⋮ -> Install app).
  setTimeout(() => btn.remove(), 20000);
}

// iOS Safari has no beforeinstallprompt event at all - show a one-time
// text tip instead, since the only way to install there is the manual
...
if (isIos() && !isInStandaloneMode() && !localStorage.getItem('shs-ios-install-tip-shown')) {
  window.addEventListener('load', () => {
    setTimeout(() => {
      const tip = document.createElement('div');
      tip.textContent = 'Tip: tap Share, then "Add to Home Screen" to install this app.';
      tip.style.cssText = `
        position: fixed;
        bottom: 20px;
        left: 20px;
      ...
      localStorage.setItem('shs-ios-install-tip-shown', '1');
