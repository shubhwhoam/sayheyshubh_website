...
// Helper functions for get-notes endpoint
...
app.get('/.netlify/functions/get-notes', async (req, res) => {
  try {
    ...
    const semesterKey = req.query.semester;
    ...
    // Sanitize notes data (hide URLs for locked notes)
    ...
    if (error.message && (error.message.includes('authorization') || error.message.includes('authentication'))) {
...
// Secure notes endpoint - returns note URL for authenticated users with access
app.get('/.netlify/functions/secure-notes', async (req, res) => {
  try {
    // Verify authentication
    const decodedToken = await verifyFirebaseToken(req.headers.authorization);
    const authenticatedUserId = decodedToken.uid;
    ...
    for (const semesterKey in notesConfig) {
      ...
      for (const subjectName in semester.subjects) {
        ...
        for (const unit of subject.units) {
          if (unit.noteUrl) {
            const unitNoteId = extractNoteId(unit.noteUrl);
            if (unitNoteId === noteId) {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('- GET  /.netlify/functions/get-notes');
  console.log('- GET  /.netlify/functions/secure-notes');
