...
function renderCommentHtml(comment, ratingSummary) {
  ...
  const reviewRatingHtml = hasRating ? `
    ...
    <span itemprop="itemReviewed" itemscope itemtype="https://schema.org/Course">
      <meta itemprop="name" content="BSc Botany Honours Notes">${aggregateRatingHtml}
    </span>` : '';
...
async function handler(event, context) {
  ...
  const possiblePaths = [
    path.resolve(__dirname, '../../botany.html'),
    path.resolve(__dirname, '../botany.html'),
    path.resolve('botany.html'),
    path.resolve('./botany.html')
  ];
  ...
  try {
    ...
    const allForPage = await db.collection('comments').where('page', '==', 'botany').where('parentId', '==', null).get();
    let ratingSum = 0;
    let ratingCount = 0;
    ...
    const countSnapshot = await db.collection('comments').where('page', '==', 'botany').where('parentId', '==', null).count().get();
